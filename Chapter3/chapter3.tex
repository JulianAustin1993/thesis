...
Finally we discuss Gaussian processes that are used in the CPACE framework to model correlation between functions.
...
The conditional expectation technique described above from \citep{yao_functional_2005} alleviates the issue of poor integral approximation from sparsely observed data when the estimated covariance surface is a relatively good fit to the true covariance surface.
...
A large number of basis functions reduce bias in the estimator $\hat{\chi}_i(t)$, but the variance of this estimator may be unacceptably high.
...
\citeauthor{hyndman_robust_2007} in \citep{hyndman_robust_2007} suggest to assume the principal component scores, $\xi_{ik}$, follow independent univariate time series models.
...
\begin{align*}
	...
	k&: \left(\ve{s}, \vesup{s}{\prime} \right) \mapsto \text{Cov}\left(\xi(\ve{s}), \xi(\vesup{s}{\prime})\right)
\end{align*}
...
Whilst non-stationary kernels have been considered by combining locally stationary kernels, \cite{genton_classes_2001}.
...
With almost all covariance functions and especially non-stationary covariances there are typically hyperparameters which must be estimated from the data.
